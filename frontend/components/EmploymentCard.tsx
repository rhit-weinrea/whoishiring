...
export default function EmploymentCard({ listing, onPinToggle, isPinned }: EmploymentCardProps) {
  ...
  const cleanDescription = (text: string) => {
    ...
    return text
      ...
      .filter(line => {
        ...
        if (lowered.startsWith('apply url:') || lowered.startsWith('apply:') || lowered.startsWith('visa sponsorship:')) return false;
  ...
  const computeTimeElapsed = (timestamp: string) => {
    ...
    if (dayGap > 7) return `${Math.floor(dayGap / 7)}w past`;
    if (dayGap > 0) return `${dayGap}d past`;
    if (hourGap > 0) return `${hourGap}h past`;
    return 'moments ago';
  };
  ...
  return (
    <div className="box rounded-xl p-8 border-2 border-[var(--outline)] hover:border-smoky-rose-500 transition-all duration-300 group">
      <div className="flex justify-between items-start mb-4">
        <div className="flex-1">
          <div className="flex flex-col gap-1">
            <span className="font-semibold text-lg text-[var(--foreground)] flex items-center gap-2">
          ...
          <div className="flex items-center gap-3 text-sm text-[var(--muted)]">
            <span className="flex items-center gap-2">
              <i className="bi bi-geo-alt text-[var(--foreground)]" aria-hidden="true" />
              {listing.location}
            </span>
            {listing.remote && (
              <span className="bg-[var(--background)] text-[var(--foreground)] px-2 py-1 rounded-full text-xs font-bold flex items-center gap-2 border border-[var(--outline)]">
        ...
        {onPinToggle && (
          ...
          >
            <i
              className={`bi ${isPinned ? 'bi-star-fill' : 'bi-star'} text-[var(--foreground)]`}
              aria-hidden="true"
            />
          </button>
        )}
      ...
      {listing.tech && listing.tech.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-4">
          {listing.tech.map(tag => (
            <span
              key={tag}
              className="px-2 py-1 text-xs font-semibold border border-[var(--outline)] rounded bg-[var(--background)] text-[var(--muted)]"
            >
              {tag}
            </span>
          ))}
      ...
      <div className="flex justify-between items-center pt-4 border-t border-[var(--outline)]">
        <div className="flex items-center gap-4 text-xs text-[var(--muted)]">
          <span className="flex items-center gap-2">
            <i className="bi bi-clock text-[var(--foreground)]" aria-hidden="true" />
            {computeTimeElapsed(listing.posted_at)}
          </span>
          {listing.salary && (
            <span className="bg-[var(--background)] text-[var(--foreground)] px-2 py-1 rounded font-semibold flex items-center gap-2 border border-[var(--outline)]">
              <i className="bi bi-cash-stack text-[var(--foreground)]" aria-hidden="true" />
              {listing.salary}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-3">
          {listing.hnItemId && (
            ...
            >
              <i className="bi bi-link-45deg text-[var(--foreground)]" aria-hidden="true" />
            </a>
          )}
          {applyUrl && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="px-3 py-2 border border-[var(--outline)] rounded-lg text-xs font-semibold text-[var(--foreground)] hover:bg-smoky-rose-500 hover:text-white transition-colors"
